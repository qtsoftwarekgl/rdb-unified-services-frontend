...
import { BusinessActivitiesCertificate, Certificate } from '@/types/models/certificate';
...
export const generateBusRegistrationEnterpriseCertificatePdf = (certificate: Certificate, isFullCertificate:boolean) => {
...
const addBorders = () => {
  ...
  // Enterprise name
  doc.setFont("helvetica", "bold");
  doc.text("Enterprise Name : ", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 60);

  doc.setFont("helvetica", "normal");
  doc.text(certificate?.enterpriseName ||  "", PAGE_MARGIN + INNER_PADDING + 35, PAGE_MARGIN + INNER_PADDING + 60);
 
  doc.setFont("helvetica", "bold");
  doc.text("Business Name : ", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 70);

  doc.setFont("helvetica", "normal");
  doc.text(certificate?.businessName || "", PAGE_MARGIN + INNER_PADDING + 35, PAGE_MARGIN + INNER_PADDING + 70);
  ...
  doc.setFontSize(12);
  doc.setFont("helvetica", "bold");
  doc.text("Head Office Address", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 85);
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  doc.text("Address : ", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 100);

  doc.setFont("helvetica", "normal");
  doc.text(getCompanyAddress(certificate), PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 110);

  doc.setFont("helvetica", "bold");
  doc.text("Email : ", PAGE_MARGIN + INNER_PADDING + 90, PAGE_MARGIN  + INNER_PADDING + 100);

  doc.setFont("helvetica", "normal");
  doc.text(certificate?.registeredOfficeAddress?.email || "", PAGE_MARGIN + INNER_PADDING + 90, PAGE_MARGIN + INNER_PADDING + 110);

  doc.setFont("helvetica", "bold");
  doc.text("Phone : ", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 120);

  doc.setFont("helvetica", "normal");
  doc.text(certificate?.registeredOfficeAddress?.phone || "", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 130);

  // Po box next to phone number
  if(certificate?.registeredOfficeAddress?.poBox){
  doc.setFont("helvetica", "bold");
  doc.text("PO Box : ", PAGE_MARGIN + INNER_PADDING + 90, PAGE_MARGIN + INNER_PADDING + 120);

  doc.setFont("helvetica", "normal");
  doc.text(certificate?.registeredOfficeAddress?.poBox || "", PAGE_MARGIN + INNER_PADDING + 90, PAGE_MARGIN + INNER_PADDING + 130);
  }
  // ================== BUSINESS OWNER ==================
  doc.setFontSize(12);
  doc.setFont("helvetica", "bold");
  doc.text("Business Owner", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 145);

  doc.setDrawColor(205, 207, 209);
  doc.setLineWidth(0.2);
  doc.line(PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 150, PAGE_WIDTH - PAGE_MARGIN - INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 150);

  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text("Name : ", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 160);

  doc.setFont("helvetica", "normal");
  doc.text(certificate?.businessOwner?.name?.replace("null","") || "", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 170);

  doc.setFont("helvetica", "bold");
  doc.text("Phone number : ", PAGE_MARGIN + INNER_PADDING + 90, PAGE_MARGIN + INNER_PADDING + 160);

  doc.setFont("helvetica", "normal");
  doc.text(certificate?.businessOwner?.phoneNumber || "", PAGE_MARGIN + INNER_PADDING + 90, PAGE_MARGIN + INNER_PADDING + 170);

  // Phone number
  doc.setFont("helvetica", "bold");
  doc.text("Document ID : ", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 180);

  doc.setFont("helvetica", "normal");
  doc.text(certificate?.businessOwner?.documentId || "", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 190);

  // Email
  doc.setFont("helvetica", "bold");
  doc.text("Email : ", PAGE_MARGIN + INNER_PADDING + 90, PAGE_MARGIN + INNER_PADDING + 180);
    
  doc.setFont("helvetica", "normal");
  doc.text(certificate?.businessOwner?.email || "", PAGE_MARGIN + INNER_PADDING + 90, PAGE_MARGIN + INNER_PADDING + 190);

  // Gender

  doc.setFont("helvetica", "bold");
  doc.text("Gender : ", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 200);

  doc.setFont("helvetica", "normal");
  doc.text(certificate?.businessOwner?.gender || "", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 210);
  

  // Address
  doc.setFont("helvetica", "bold");
  doc.text("Address : ", PAGE_MARGIN + INNER_PADDING + 90, PAGE_MARGIN + INNER_PADDING + 200);

  doc.setFont("helvetica", "normal");
  doc.text(certificate?.businessOwner?.address?.replace("null", "") || "", PAGE_MARGIN + INNER_PADDING + 90, PAGE_MARGIN + INNER_PADDING + 210);
  ...
  const contentY = isFullCertificate ? 50 : 0;

  doc.setFont("helvetica", "bold");
  doc.text("Main Business Activity", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 10 + contentY);

  doc.setFont("helvetica", "normal");
  doc.text("Code", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 20 + contentY);
  doc.text(":", PAGE_MARGIN + INNER_PADDING + 10, PAGE_MARGIN + INNER_PADDING + 20 + contentY);
  doc.text(certificate?.mainBusinessActivityCode || "", PAGE_MARGIN + INNER_PADDING + 15, PAGE_MARGIN + INNER_PADDING + 20 + contentY);

  // Display Date next to the code
  doc.text("Date", PAGE_MARGIN + INNER_PADDING + 50, PAGE_MARGIN + INNER_PADDING + 20 + contentY);
  doc.text(":", PAGE_MARGIN + INNER_PADDING + 60, PAGE_MARGIN + INNER_PADDING + 20 + contentY);

  doc.text(certificate?.mainBusinessActivityDate || "", PAGE_MARGIN + INNER_PADDING + 65, PAGE_MARGIN + INNER_PADDING + 20 + contentY);

  doc.text("Description : ", PAGE_MARGIN + INNER_PADDING, PAGE_MARGIN + INNER_PADDING + 30 + contentY);
  doc.text(certificate?.mainBusinessActivity || "", PAGE_MARGIN + INNER_PADDING + 20, PAGE_MARGIN + INNER_PADDING + 30 + contentY);
  ...
  const addBusinessActivitiesPage = (activities: BusinessActivitiesCertificate[], startY: number) => {
    ...
    activities.forEach((activity: BusinessActivitiesCertificate, index: number) => {
      if (y + ROW_HEIGHT > PAGE_HEIGHT - CONTENT_MARGIN - PAGE_MARGIN - 65) { // Check if the content fits on the page
        doc.addPage();
        addBorders();
        footer();
  ...
  const y = 75 + contentY; // Starting position for y
